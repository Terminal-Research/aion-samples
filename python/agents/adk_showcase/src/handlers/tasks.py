...
async def task_handler(ctx: AionInvocationContext) -> AsyncGenerator[Event, None]:
    ...
    """
    thread = Thread.from_context(ctx.aion_runtime_context)
    task_id, context_id = _inbound_ids(ctx)
